...
class _AdminChatScreenState extends State<AdminChatScreen> {
  ...
  // Future<void> _fetchHRName() async {
  //   final hrDoc = await FirebaseFirestore.instance
  //       .collection('users')
  //       .doc(widget.hrId)
  //       .get();
  //
  //   if (hrDoc.exists && mounted) {
  //     setState(() {
  //       hrName = hrDoc['name'] ?? "HR";
  //     });
  //   } else {
  //     setState(() {
  //       hrName = "HR";
  ...
  //         .collection('users')
  //         .where('role', isEqualTo: 'HR')  // HR role
  //         .where('organizationName', isEqualTo: widget.orgName)
  //         // .limit(1) // optional: if you expect only one HR
  //         .get();
  //
  //     if (querySnapshot.docs.isNotEmpty && mounted) {
  //       setState(() {
  //         hrName = querySnapshot.docs.first['displayName'] ?? "HR";
  //       });
  //     } else if (mounted) {
  //       setState(() {
  //         hrName = "HR";
  ...
  Future<void> _fetchHRName() async {
    try {
      ...
      if (querySnapshot.docs.isNotEmpty && mounted) {
        // safely get the first HR document
        var data = querySnapshot.docs.first.data(); // Map<String, dynamic>

        setState(() {
          // check if 'displayName' exists
          hrName = (data.containsKey('displayName') && data['displayName'] != null)
              ? data['displayName']
              : "HR"; // fallback
        });
      } else if (mounted) {
      ...
      print("Error fetching HR: $e"); // log the exact error
      if (mounted) {
        setState(() {
          hrName = "HR"; // fallback on error
        });
      }
    }
  }
        ...
        title: Row(
          children: [
            CircleAvatar(
              ...
              child: Text(
                hrName.isNotEmpty ? hrName[0].toUpperCase() : 'H',
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<List<Message>>(
              stream: chatUtils.getMessages(widget.hrId, widget.orgName),
